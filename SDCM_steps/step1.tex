...
$\nu = n - 2$. Die Subtraktion von 2 entsteht, weil beim Schätzen der
...
Wert $t$ allein durch Zufall auftritt, wenn die Nullhypothese gilt:
\begin{equation}
    \rho_\nu(t)
    \;=\;
...
Ränder (sogenannte \emph{fat tails}), da bei wenigen Datenpunkten
extreme $t$-Werte durch Zufall häufiger auftreten. Der $p$-Wert ist
die Fläche unter $\rho_\nu$ ab dem beobachteten $t$ bis in den
jeweiligen Schwanz, also die Wahrscheinlichkeit, durch Zufall einen
mindestens so extremen $t$-Wert zu erhalten. Da $p$-Werte hier die
...
nur den entsprechenden einseitigen Schwanz (Supp.~Note~2):
\begin{equation}
    p \;=\; \int_t^{\infty} \rho_\nu(t')\,dt'
    \quad \text{für } t \geq 0,
    \qquad
    p \;=\; \int_{-\infty}^{t} \rho_\nu(t')\,dt'
    \quad \text{für } t \leq 0.
\end{equation}
